...
#### 💡 잠깐! 마케팅 논문 검색 시 흔한 오해 (OpenAlex vs AlexNet)
종종 AI나 논문을 처음 접하시는 분들이 "AlexNet에서 찾으면 되나요?"라는 질문을 하곤 합니다. 하지만 여기서 혼동하지 말아야 할 핵심 플랫폼이 있습니다.
*   **OpenAlex (오픈알렉스):** 우리가 찾던 **방대한 학술 자료의 공개 데이터베이스**입니다. 전 세계 모든 논문, 저자, 기관의 데이터를 연결한 거대한 지식 지도로, 구글 스칼라에 필적하는 검색력을 무료로 제공합니다.
*   **AlexNet의 진실:** AlexNet은 논문 검색 사이트가 아니라, 2012년에 발표되어 딥러닝의 부흥을 이끈 **유명한 AI 이미지 인식 모델의 이름**입니다. 이름이 비슷하여 OpenAlex와 오해받는 경우가 많으니 주의가 필요합니다.[^7]
*   **arXiv(아카이브)의 한계:** 컴퓨터 공학 분야에서는 필수지만, 마케팅/경영 분야의 저명한 저널 논문은 이곳에 거의 올라오지 않으므로 아래의 특화 엔진을 병행해야 합니다.

#### 📌 2026년 최상위 논문 발굴 및 레퍼런스 체크 플랫폼 (Project Standard)
이 프로젝트의 리서치 규칙([.antigravity_rules.md](file:///.antigravity_rules.md))에 따라, 심도 있는 문헌 고찰을 위해 아래의 **AI 특화 학술 엔진**을 우선적으로 활용합니다.

1.  **OpenAlex / Google Scholar:** 방대한 학술 자료의 기본 검색 및 원문 탐색을 위한 '베이스캠프'입니다.
2.  **Semantic Scholar (시맨틱 스콜라):** AI 기반 검색 및 특정 이론의 발전 방향(인용 그래프)을 추적하여 가장 근본이 되는 논문을 찾을 때 활용합니다.
3.  **Consensus (컨센서스):** 자연어 질문을 던지면 동료 평가(Peer-reviewed)를 거친 논문들만 분석하여 학계의 '과학적 합의점'을 도출해 줍니다. 팩트 체크에 최적입니다.[^8]
4.  **Elicit (엘리싯):** 여러 논문에서 실험군 크기, 측정 지표, 결론 등을 추출하여 체계적으로 표 형태로 비교 분석할 때 강력한 힘을 발휘합니다.[^9]
5.  **SSRN:** 최신 마케팅 트렌드가 반영된 출판 전 워킹 페이퍼(Working Paper)를 확보하기에 유리합니다.
