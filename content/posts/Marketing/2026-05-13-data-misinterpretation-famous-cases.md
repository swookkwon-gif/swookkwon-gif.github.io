...
## 7. 텍사스 명사수 오류 (Texas Sharpshooter Fallacy): 과녁을 나중에 그리는 마법

* **배경**: 어느 텍사스 카우보이가 헛간 벽에 총을 마구잡이로 난사한 다음, 총알이 가장 많이 몰린 곳에 다가가 과녁(Target)을 둥글게 그려 넣고는 "나는 명사수다!"라고 자랑합니다. 이것이 바로 이 유명한 통계적 착시 현상의 이름입니다.
* **데이터의 착시 (스웨덴 송전선 사례)**: 1993년 스웨덴 연구진이 고압 송전선과 인근 거주민들의 건강 연관성을 찾기 위해 25년간의 데이터를 조사했습니다. 그 결과, 송전선 300미터 이내 거주자들에게서 소아 백혈병 발병률이 무려 4배나 높게 나타났습니다.
* **해석의 오류**: 연구진은 '송전선이 소아 백혈병을 유발할 것이다'라는 가설을 미리 세우고 검증한 것이 아니었습니다. 이들은 무려 **800개가 넘는 다양한 질병 데이터를 무작위로 샅샅이 분석**했습니다. 통계학적으로(P-value 0.05 기준) 800개를 검사하면 순수한 우연만으로도 40개 정도의 질병은 비정상적으로 높거나 낮게 나타납니다. 연구진은 우연히 튀어나온 결과 하나를 집어 들고 사후에 의미를 부여하는 **다중 비교 문제(Multiple Comparisons Problem)**를 저질렀습니다. 
* **실제 진실**: 이후 철저하게 통제된 수많은 대규모 후속 연구에서는 송전선과 소아 백혈병 간의 어떠한 연관성도 발견되지 않았습니다. 

![텍사스 명사수 오류 시각화](/images/texas_sharpshooter.png)
*(빅데이터 시대의 함정: 수많은 무작위 데이터 속에서 우연히 발견된 패턴에 사후적으로 의미를 부여하는 행위)*

* **비즈니스에서의 함정**: 오늘날 빅데이터와 퍼포먼스 마케팅 현장에서 가장 자주, 흔하게 발생하는 오류입니다. 마케터가 50개의 서로 다른 광고 소재(A/B/C... 테스트)를 무작위로 라이브시킵니다. 그중 1개의 소재가 순전히 우연한 노이즈나 알고리즘의 초기 분배 오류로 높은 클릭률(CTR)을 기록합니다. 마케터는 즉시 *"역시 우리 브랜드의 타겟 고객은 이런 B급 감성을 원했어!"*라며 장황한 사후 분석 보고서를 작성합니다. 헛간 벽에 박힌 무작위 총알에 멋진 과녁을 그려 넣은 셈입니다. 데이터 마이닝 툴이 발달할수록 우리는 우연히 발견된 가짜 상관관계를 위대한 인사이트로 착각하지 않도록 극도로 주의해야 합니다.
