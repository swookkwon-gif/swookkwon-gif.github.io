...
6. 레퍼런스: 각 뉴스의 출처를 기재할 때, 홈페이지 메인 도메인(예: aitimes.com)만 적거나 임의로 축약하지 말고 **반드시 상세 기사의 전체 URL(Full 원본 URL)을 누락 없이 그대로 사용할 것.**
7. 어조: ~이다/~한다 체. 모든 URL은 본문에 텍스트 형태로 직접 노출하지 말고, 반드시 `[기사 제목](전체 상세 URL)` 형식의 마크다운 링크로 감쌀 것.
