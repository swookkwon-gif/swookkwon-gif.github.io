...
6. 출처 링크 기재 시 도메인만 적지 말고 **반드시 상세 기사의 원본 전체 URL(Full URL)을 누락 없이 그대로 사용할 것.**
7. 모든 URL은 본문에 텍스트 형태로 직접 노출하지 말고, 반드시 `[텍스트](상세 URL)` 형식의 마크다운 링크로 감쌀 것.
...
5. Top 10 심층 분석: 가장 중요한 10개 뉴스를 각각 큰 소제목(##)으로 구분.
   - 배경 설명(2~3문단) + 핵심 뉴스 + 기술적/산업적 시사점 + 향후 전망을 자연스러운 문단으로 작성.
5. 하단에 Top 10 뉴스 마크다운 테이블 (순위, 기사 제목, 중요도 점수, 선정 사유).
6. 레퍼런스: 하단 출처 표기 시 매체명과 링크가 따로 놀지 않도록 반드시 `소스: [매체명1](상세 URL1) · [매체명2](상세 URL2)` 형태로 매핑하여 작성할 것.
7. 어조: ~이다/~한다 체. 모든 출처 URL은 도메인으로 축약하지 말고 반드시 전체 기사 URL(Full URL)을 사용하되, 본문에 그대로 노출하지 말고 마크다운 링크로 숨길 것.
8. **[최소 분량] 반드시 1,500단어(한글 3,000자) 이상 작성할 것.** Top 10 각각을 최소 150단어 이상 심층 분석해.
...
6. 포스트 하단에 '## 📚 참고자료' 섹션 추가. 출처 기재 시 **상세 원본 전체 URL(Full URL)을 누락 없이 사용할 것.**
7. URL은 텍스트 형태로 직접 노출하지 말고, 반드시 `[텍스트](상세 URL)` 마크다운 링크를 사용할 것.
...
def research_daily_top3(
    ...
    """
    Daily Top 10 AI 뉴스 리서치 → 블로그 포스트 발행.
