...
def run_daily_ai_deep_research():
    ...
    print(f"\n[Step 3] Top 10 뉴스 기반 심층 마크다운 작성")
    writing_prompt = """
위에서 리서치하여 저장한 'Top 10 뉴스 소스'를 바탕으로 아주 상세한 기술 블로그 포스트를 한국어로 작성해줘.

[작성 가이드라인]
1. 메타 데이터 생성 (반드시 첫 두 줄에 작성):
   - 첫 번째 줄은 "TITLE: [제목]" 형식으로 작성하되, 대괄호 `[]`는 빼고 제목 텍스트만 적어. 제목은 가장 중요한 Top 10 뉴스의 핵심 팩트(키워드)만 콤마로 나열해 (예: TITLE: 구글 앤스로픽 400억 달러 투자, MS-오픈AI 계약 변경, 메타 AWS CPU 대규모 도입). '시사하는 바' 같은 주관적 문구는 절대 넣지 마.
...
5. Top 10 주요 뉴스 및 중요도 평가 테이블 (반드시 마크다운 표 형식 사용):
   ...
   - 이 중 앞서 다룬 10가지가 왜 Top 10으로 선정되었는지, 그리고 나머지 기사들은 왜 순위가 밀렸는지 '선정 사유'에 매우 구체적인 논거를 들어 기술해. (단순히 '중요하다'가 아니라, 예를 들어 '6천억 달러 규모의 펀딩 지연이 국방부의 일상적인 AI 도입 뉴스보다 산업적 파급력이 훨씬 크기 때문'과 같이 구체적인 비교 우위와 자본/기술적 파급력을 기준으로 중요도를 평가할 것). 텍스트가 깨지지 않도록 표 내부의 줄바꿈은 `<br>` 태그를 사용해.
6. 레퍼런스 및 주석:
