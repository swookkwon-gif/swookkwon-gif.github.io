...
오늘({now_kst.strftime('%Y년 %m월 %d일')}) 기준으로 지난 24~48시간 동안 발생한 전 세계 AI 기술 및 산업 뉴스 중, **가장 파급력이 크고 중요한 단 1가지 뉴스(오늘의 헤드라인 1개)**를 구글 검색 기능을 활용하여 심층 리서치하세요.
...
1. 메타 데이터 생성 (반드시 첫 두 줄에 작성):
   - 첫 번째 줄은 "TITLE: [제목]" 형식으로 작성하되, 대괄호 `[]`는 빼고 제목 텍스트만 적어. 뉴스 내용을 구체적이고 명확하게 알 수 있도록 기존보다 1.5배 이상 길고 서술적인 문장으로 작성해 (예: TITLE: 구글, 제미니 1.5 플래시 업데이트를 통해 멀티모달 컨텍스트 윈도우 대폭 확장). '시사하는 바' 같은 주관적 문구는 넣지 마.
   - 두 번째 줄은 "EXCERPT: [요약문]" 형식으로 전체 포스트를 2~3문장(약 100~150자)으로 요약한 프리뷰 텍스트를 적어.
2. 본문 서식: 가독성을 높이기 위해 텍스트 단락 구분을 명확히 하고, 적절한 줄바꿈을 사용할 것.
3. 단일 심층 분석 (가장 중요한 1개 뉴스):
   - 해당 1개의 뉴스를 큰 소제목(##)으로 작성하고, 핵심 요약(2~3줄)에 이어서 기술적 및 산업적 시사점을 아주 깊이 있게 분석하여 긴 줄글로 자연스럽게 작성해. (단, '[소식 요약]', '[시사점]' 같은 영역별 제목이나 괄호 표기는 절대 사용하지 말고 자연스러운 문단으로 구성할 것)
...
5. 레퍼런스 및 주석 (중요: URL 링크 필수 포함!):
   ...
   - **절대 누락 금지**: 구글 검색 결과에서 확인한 원본 기사의 실제 웹 주소(URL, https://...)를 마크다운 링크 문법 `(URL)` 안에 반드시 텍스트로 명시적으로 출력해야 해! (AI가 링크를 생략하는 경우가 많으니 특별히 주의할 것)
...
"""
    ...
    display_title = f"Daily Top 1: {now_kst.strftime('%m월 %d일')} 주요 AI 헤드라인"
    display_excerpt = "오늘의 가장 핵심적인 글로벌 AI 뉴스 1건을 심층 분석합니다."
