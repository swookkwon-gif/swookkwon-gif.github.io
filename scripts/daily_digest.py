...
3. **주요 뉴스 선별 및 정렬**:
   - 뉴스레터/RSS에서 수집된 [3점 이상 주요 기사] 중 가장 중요한 **상위 3~5개 기사만**을 선별하여 상세한 메인 뉴스로 작성하세요. (딥 리서치 내용과는 별개로 RSS에서 3~5개를 뽑아야 합니다).
...
5. **기타 뉴스**: 상위 3~5개로 선정되지 않은 나머지 기사는 모두 `## 📌 기타 뉴스 모아보기` 섹션에서 소스별로 그룹핑하세요.
   - 소스 홈페이지 링크는 걸지 말고 `### 🔹 소스: 매체명` 형식으로 작성하세요.
   - 각 기사는 `* **[기사 제목](기사 URL)**: 요약 한 줄` 형태로 작성하며, URL이 없는 기사도 제외하지 말고 링크 없이 `* **기사 제목**: 요약 한 줄` 형태로 작성하세요.
6. **post_title 생성 규칙**: 날짜나 'AI 데일리 다이제스트' 등의 단어 없이, **딥 리서치의 핵심 주제 1개와 RSS 주요 뉴스 1~2개의 핵심 내용을 모두 포함하여 기존보다 1.5배 이상 길고 상세하게** 콤마(,)로 연결하여 작성하세요. (예: "엔비디아 400억 달러 투자로 AI 생태계 지배력 강화, 오픈소스 AI 에이전트 자가 복제 능력 급증 우려, 구글 제미니 1.5 플래시 멀티모달 컨텍스트 확장")
7. **top_topics 생성 규칙**: 파일명에 사용할 수 있도록, 상위 3개 토픽을 영문 소문자와 하이픈(-)만 포함한 짧은 영문 키워드로 제공하세요. (예: ["nvidia-investment", "ai-agent-hacking", "google-gemini"])
"""
