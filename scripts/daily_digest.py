...
3. **주요 뉴스 선별 및 정렬**:
   - 뉴스레터/RSS에서 수집된 [3점 이상 주요 기사] 중 **여러 소스에서 중복으로 다루어진 기사를 최우선으로 선정**하여 핵심 기사 **최대 15개**를 선별하고 상세한 메인 뉴스로 작성하세요. (딥 리서치 내용과는 별개로 선별해야 합니다).
   - 각 기사의 제목에는 아이콘(이모지)을 절대 사용하지 마세요.
4. **상위 주요 메인 뉴스 포맷 (최대 15개)**:
...
5. **기타 뉴스**: 위에서 선정된 주요 뉴스를 제외한 나머지 기사 중 **중요도가 낮거나 단순 반복되는 기사는 과감히 제외(필터링)**하여 기타 뉴스 개수를 대폭 줄이세요. 선별된 소수의 단신만 `## 📌 기타 단신 모아보기` 섹션에서 소스별로 그룹핑하세요. (최대 10개 이내로 제한)
