...
3. **메인 뉴스(Top 10) 포맷**:
   ...
   - 반드시 **제목 바로 아래에 줄바꿈(Enter) 2번**을 넣어 문단을 명확히 분리하세요.
   - 본문 2-4문장 + 핵심 수치 불릿 강조
   - 뉴스 하단 출처 표기 시, 반드시 소스에 있는 URL을 추출하여 링크로 연결하세요 (본문 직후 빈 줄 없이 `<br>` 사용):
     `<br><small style="color: #888;">소스: 7min.ai · AITimes &nbsp;|&nbsp; 🔗 [원문 보기](실제 URL 삽입) · [원문 2](URL)</small>`
   - 각 기사가 끝난 후 반드시 빈 줄을 추가하고, `---` 구분선을 넣은 뒤, 다시 빈 줄을 추가하세요.
4. **기타 뉴스 (소스별 그룹핑)**:
   - Top 10에 들어가지 못한 기사들은 하단의 `## 📌 기타 뉴스 모아보기` 섹션에 통합하세요.
   - 소스 기준으로 그룹화하여 아래 포맷으로 작성하세요.
     ### 🔹 소스: 소스 이름
     * **[기사 제목 1](반드시 실제 URL 삽입)**: 요약 한 줄 (URL이 없는 기사는 링크 없이 텍스트로 기재)
5. **출력 형식**: 마크다운 문법을 완벽히 준수하고, 텍스트가 한 줄로 뭉치지 않도록 문단 사이에 반드시 줄바꿈(\\n\\n)을 포함하세요. 백틱(```)은 사용하지 마세요."""
