...
이처럼 실소(失笑)를 자아내는 빤한 통계의 오류를 비즈니스 테이블의 똑똑한 마케터들은 매일 같이 저지르고 있다. 이를 가장 명징하게 입증하는 것이 **두 쌍의 월별 꺾은선 추이 그래프**의 대조다.

*   **그래프 A (아이스크림 📈 vs 익사자 📈):** 미국의 월별 아이스크림 생산량과 월별 익사자 추이 곡선을 1년 단위로 겹쳐 그려보면, 여름인 7~8월에 완벽하게 솟구쳤다가 겨울에 동반 하락하는 대칭을 이룬다. "아이스크림이 익사를 일으킨다"고 믿고 마트의 아이스크림을 단속하는 사람은 없다. 진짜 원인은 두 수치를 모두 끌어올린 **'기온(여름)'이라는 제3의 교란 변수**이기 때문이다.
