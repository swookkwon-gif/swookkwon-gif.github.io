# 📖 디지털 마케팅의 거짓말: 집필 백로그 (최종 업데이트)

## 📊 진행 상태 요약
- **새로운 타겟 챕터:** 9개
- **기존 포스트 연동 완료:** 12개 (Hoffman, Ramos 포스트 추가 완료)
- **전체 챕터 수:** 19개

## 🗂️ 집필 상태 데이터베이스 (최근 완료 항목 포함)

| ID | 구분 | 챕터명 (가제) | 상태 | 연관 폴더 | 핵심 레퍼런스 | AI 태스크 상태 |
|:---|:---|:---|:---:|:---|:---|:---:|
| C17 | Part 2 | 역사상 최대의 사기: ADSCAM과 추적의 위협 | **완료** | Marketing | Bob Hoffman (ADSCAM) | 포스팅완료 |
| C18 | Part 2 | 사라진 800억 달러: 디지털 광고 사기의 실태 | **완료** | Marketing | Andreas Ramos (Ad Fraud) | 포스팅완료 |
| C19 | Part 3 | 광고 플랫폼의 '숨겨진 설정': 낭비의 구조 | **완료** | Marketing | Ramos, Hoffman | 포스팅완료 |
| C08 | Part 2 | MMM(마케팅 믹스 모델링)의 부활과 한계 | **완료** | Data | Robyn, LightweightMMM | 포스팅완료 |
| C14 | Part 3 | 앱 마케팅(UAC)의 허수 트래픽과 Fraud 방어 | 대기 | Marketing | AppsFlyer Fraud Report | 미배정 |
| C15 | Part 3 | 데이터 신호 설계(Signal Design) | 대기 | Data | First-party Data Strategy | 미배정 |
| C16 | Part 4 | CEO와 CMO의 데이터 동상이몽 | 대기 | Marketing | HBR Case Study | 미배정 |

---
## 📚 전체 참고 문헌 리스트 (Reference List)
- **Bob Hoffman**, *ADSCAM: How Online Advertising Gave Birth to One of History's Greatest Frauds* (2022)
- **Bob Hoffman**, *BadMen: How Advertising Went From A Minor Annoyance To A Major Menace* (2017)
- **Andreas Ramos**, *Ad Fraud: How to Prevent Digital Ad Fraud*
- **Blake, Nosko, Tadelis (2015)**, *Consumer Heterogeneity and Paid Search Effectiveness*, eBay Case Study.
- **Tim Hwang**, *Subprime Attention Crisis: Advertising and the Time Bomb at the Heart of the Internet* (2020)
- **Lazer et al. (2014)**, *The Parable of Google Flu: Traps in Big Data Analysis*, Science.
- **Optmyzr (2024)**, *The State of Performance Max Report*.
- **SpiderAF (2024)**, *Digital Ad Fraud Trend Report*.

---
*마지막 업데이트: 2026-05-12*
